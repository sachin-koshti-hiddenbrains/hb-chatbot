console.log("here is the sahchin");
...
function App() {
  ...
  const ENV_API_URL = config.APP_EndPoint;
  ...
  // const [chatWindow, setChatWindow] = useState(`${config.ChatWindow.TypeOfChatWindow}`)
...
chatWindow = "ChatScreen" }
  ...
  moment.locale(config.DateFormat.Locale);
  useEffect(() => {
    bottomRef.current?.scrollIntoView({ behavior: "smooth" });
    if (conversation.length > 0) {
      const Last20 = conversation.slice(-40);
      if (config.UserId == 0) {
        sessionStorage.setItem(`${config.UserId}`, JSON.stringify(Last20));
      } else {
        localStorage.setItem(`${config.UserId}`, JSON.stringify(Last20));
      }
    }
  }, [conversation]);
  ...
  useEffect(() => {
    if (
      config.UserId == 0 &&
      sessionStorage.getItem(`${config.UserId}`)?.length > 0
    ) {
      let RestoreFromSessionStorage = JSON.parse(
        sessionStorage.getItem(`${config.UserId}`)
      );
      setConversation(RestoreFromSessionStorage);
    } else if (
      config.UserId != 0 &&
      localStorage.getItem(`${config.UserId}`)?.length > 0
    ) {
      let RestoreFromLocalStorage = JSON.parse(
        localStorage.getItem(`${config.UserId}`)
      );
...
const speech = (sys)=>{
  ...
  if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
    let speechRecognition = new (window.webkitSpeechRecognition || window.SpeechRecognition)();
    let final_transcript = "";
  ...
  const handleSubmit = async (e, sampleQues) => {
    ...
    //   second: '2-digit',
    // });
    const formattedDateTime = moment().format(config.DateFormat.Format);
  ...
  const DateStickyBar = ({ index }) => {
    ...
    if (index == 0 || moment(conversation[index].time).format('YYYY-MM-DD') != moment(conversation[index - 1].time).format('YYYY-MM-DD')) {
      if (moment(conversation[index].time).format('YYYY-MM-DD') == moment().format('YYYY-MM-DD')) {
        ...
        date = moment(conversation[index].time).format('DD MMMM YYYY')
      }
      return (
              ...
              <div className="row">
                ...
                <div className="chat-footer">
                  <div className="msg-send">
                    <img src={config.ChatBotProfile} alt="" width="30" />
                    <input
                      type="text"
                      name="input_text"
